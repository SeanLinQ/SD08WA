...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_BRIGHTNESS_MIN, DP_TYPE_VALUE},
  {DPID_LED_TYPE, DP_TYPE_ENUM},
};
...
void all_data_update(void)
{
  ...
  mcu_dp_value_update(DPID_BRIGHTNESS_MIN,当前最小亮度); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_LED_TYPE,当前光源类型); //枚举型数据上报;

 */
	mcu_dp_bool_update(DPID_SWITCH_1,Device_Param.led_switch); //BOOL型数据上报;
  mcu_dp_value_update(DPID_BRIGHT_VALUE,Device_Param.bright_value); //VALUE型数据上报;
  mcu_dp_value_update(DPID_BRIGHTNESS_MIN,Device_Param.brightness_min); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_LED_TYPE,Device_Param.led_type); //枚举型数据上报;
}
...
static unsigned char dp_download_switch_1_handle(const unsigned char value[], unsigned short length)
{
  ...
  else
  {
	...
	Dimmer_Save_Prmtr();
  Device_Param.led_switch = switch_1;
...
/*****************************************************************************
函数名称 : dp_download_brightness_min_handle
功能描述 : 针对DPID_BRIGHTNESS_MIN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_brightness_min_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为VALUE
  unsigned char ret;
  unsigned long brightness_min;
  
  brightness_min = mcu_get_dp_download_value(value,length);
  /*
  //VALUE类型数据处理
  ...
  */
	...
	//if(workMode == MODE_BRIGHT_SET)
	{
		Device_Param.brightness_min = brightness_min;
		if(brightness_min == 10)
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_value_update(DPID_BRIGHTNESS_MIN,brightness_min);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_led_type_handle
功能描述 : 针对DPID_LED_TYPE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led_type_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为ENUM
  unsigned char ret;
  unsigned char led_type;
  
  led_type = mcu_get_dp_download_enum(value,length);
  switch(led_type)
  {
    case 0:
  ...
  Device_Param.led_type = led_type;
	
	Dimmer_Save_Prmtr();
  //处理完DP数据后应有反馈
  ret = mcu_dp_enum_update(DPID_LED_TYPE,led_type);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
  ...
  ***********************************/
  ...
  switch(dpid)
  {
    ...
    case DPID_BRIGHT_VALUE:
      ...
      break;
    case DPID_BRIGHTNESS_MIN:
      //最小亮度处理函数
      ret = dp_download_brightness_min_handle(value,length);
      break;
    case DPID_LED_TYPE:
      //光源类型处理函数
      ret = dp_download_led_type_handle(value,length);
      break;
