...
//  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
  ...
  mcu_dp_bool_update(DPID_LED_SWITCH_1,当前开关); //BOOL型数据上报;
  mcu_dp_value_update(DPID_BRIGHT_VALUE_1,当前亮度值); //VALUE型数据上报;
  mcu_dp_value_update(DPID_BRIGHTNESS_MIN_1,当前最小亮度); //VALUE型数据上报;
  mcu_dp_enum_update(DPID_LED_TYPE_1,当前光源类型); //枚举型数据上报;
